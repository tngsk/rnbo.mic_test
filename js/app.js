async function setup() {
  const patchExportURL = "export/weprobe.test_mic.export.json";
...
function makeSliders(device) {
  ...
  device.parameterChangeEvent.subscribe((param) => {
    if (!isDraggingSlider) uiElements[param.name].slider.value = param.value;
...
function attachOutports(device) {
  const outports = device.messages.filter(
    (message) => message.type === RNBO.MessagePortType.Outport
  );
